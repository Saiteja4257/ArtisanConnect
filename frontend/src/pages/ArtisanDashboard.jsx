...
const ArtisanDashboard = () => {
  ...
  const completedOrders = directOrders.filter(order => order.status === 'completed' || order.status === 'delivered');
  ...
  return (
    <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card><CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium">Open Direct Orders</CardTitle><Clock className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{isLoadingOrders ? <Loader2 className="h-6 w-6 animate-spin" /> : openOrders.length}</div></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium">Approved Direct Orders</CardTitle><Truck className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{isLoadingOrders ? <Loader2 className="h-6 w-6 animate-spin" /> : approvedOrders.length}</div></CardContent></Card>
            ...
            <CardContent>
              {isLoadingOrders ? <div className="flex justify-center p-4"><Loader2 className="h-6 w-6 animate-spin" /></div> :
               ...
               (openOrders.length > 0 || approvedOrders.length > 0) ? (
                <Table>
                  <TableHeader><TableRow><TableHead>Product</TableHead><TableHead>Quantity</TableHead><TableHead>Buyer</TableHead><TableHead>Status</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {[...openOrders, ...approvedOrders].map((order) => (
                      <TableRow key={order._id}>
                        <TableCell className="font-medium">{order.productId?.name || 'N/A'}</TableCell>
                        <TableCell>{order.quantity} {order.productId?.unit}</TableCell>
              ...
              ) : <p className="text-center text-muted-foreground py-4">No incoming direct orders for your products yet.</p>}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="lg:col-span-2">
          <Card>
            ...
            <CardContent>
                    ...
                    <TableBody>
                      {completedOrders.map((order) => (
                          <TableRow key={order._id}>
                            <TableCell className="font-medium">{order.productId?.name || 'N/A'}</TableCell>
                            <TableCell>{order.buyer?.name || 'N/A'}</TableCell>
                            <TableCell>{order.buyer && order.buyer.address ? [order.buyer.address.street, order.buyer.address.city, order.buyer.address.zipCode].filter(Boolean).join(', ') : 'N/A'}</TableCell>
                            <TableCell>{order.quantity} {order.productId?.unit}</TableCell>
                            <TableCell className="capitalize">{order.status}</TableCell>
              ...
              ) : <p className="text-center text-muted-foreground py-4">No completed direct orders yet.</p>}
            </CardContent>
          </Card>
        </div>
      </div>
